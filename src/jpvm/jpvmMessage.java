...
class jpvmMessage {
	...
	public void send(jpvmSendConnection conn) throws jpvmException {
		...
		try {
			...
			if(messageTagArray != null){
				strm.writeInt(-1);
				strm.writeInt(messageTagArray.length);
				for (int i = 0; i < messageTagArray.length; i++) {
					strm.writeInt(messageTagArray[i]);
				}
			}
			else
			{
	...
	public void recv(jpvmRecvConnection conn) throws jpvmException {
		...
		try {
			int firstInt = strm.readInt();
			if(firstInt < 0){
			     int tagArrayLenght = strm.readInt();
			     this.messageTagArray = new int[tagArrayLenght];
			     for (int i = 0; i < tagArrayLenght; i++) {
			    	 this.messageTagArray[i] = strm.readInt();
				}
			}
			else
			{
